...
typedef struct s_node
{
	...
	char	**args;
	char	*command;
	int		type;			// (1 exec) (2 pipe) (3 >) (4 >>) (5 read input)
}	t_node;
...
void	init_nodes(t_node *root, t_node *output, t_node *input, t_node *input_file, t_node *output_file, t_node *left, t_node *right);
int		fork_check(void);
...
int main(void)
{
	...
	head = &root;
	init_nodes(&root, &output, &input, &input_file, &output_file, &left, &right);
	pid = fork_check();
	if (pid == 0)
	{
...
{
	char *arr[] = {"cat", NULL};
	char *arr2[] = {"grep", "int", NULL};

	/*
						root
			input						output
	in_file		 left			out_file		right
	*/
	root->left = input;
	root->right = output;
	root->args = NULL;
	...
	input->right = left;
	input->left = input_file;
	input->args = NULL;
	input->command = NULL;
	input->type = 2;

	input_file->right = NULL;
	input_file->left = NULL;
	input_file->args = NULL;
	input_file->command = "input.c";
	input_file->type = 5;
	...
	output->right = right;
	output->left = output_file;
	output->args = NULL;
	output->command = NULL;
	output->type = 4;

	output_file->right = NULL;
	output_file->left = NULL;
	output_file->args = NULL;
	output_file->command = "random.txt";
	output_file->type = 0;
	...
	right->type = 1;
}		// (1 exec) (2 pipe) (3 >) (4 >>) (5 read input)
...
void	redirection_file(t_node *node, char **env)
{
	...
	file_fd = open_check(node->left->command, node->type);	//	3 == > , 4 == >>
